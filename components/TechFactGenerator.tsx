...
const TechFactGenerator: React.FC = () => {
    ...
    try {
      ...
      if (typeof process.env.API_KEY === 'undefined' || process.env.API_KEY === "") {
        ...
        // To make it clear in UI, if not set.
         setError("Gemini API Key is not configured. Please contact administrator.");
         setIsLoading(false);
         return;
      }
      ...
      console.error("Failed to fetch tech fact:", err);
      setError(err.message || "Could not fetch a tech fact at this time. Please try again later.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <div className="p-6 md:p-8 rounded-lg text-center">
      ...
      >
        {isLoading ? 'Loading...' : 'Discover Fact'}
      </button>
